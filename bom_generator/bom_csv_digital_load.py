...
"""
    ...
    Grouped By: Manufacturer, PartNumber, Value
    Sorted By: Ref
    Fields: Refeference(s), Manufacturer, PartNumber, Value, Footprint, Quantity
...
def myEqu(self, other):
    ...
    """
    result = True
    if self.getField("PartNumber") != other.getField("PartNumber"):
        result = False
...
columns = ['Ref', 'Manufacturer', 'PartNumber', 'Val', 'Footprint', 'Q']
...
writerow( out, ['Source: {}'.format(net.getSource())] )
writerow( out, ['Date: {}'.format(net.getDate())] )
writerow( out, ['Tool: {}'.format(net.getTool())] )
writerow( out, ['Generator: {}'.format(sys.argv[0])] )
writerow( out, ['Qtot {}'.format(len(components))] )
writerow( out, [] )
...
for group in grouped:
    ...
    # columns = ['Ref', 'Manufacturer', 'PartNumber', 'Val', 'Footprint', 'Q'] + sorted(list(columnset))
    row.append( refs );
